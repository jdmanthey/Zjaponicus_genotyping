...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/38_zjaponicus"
	directory_name <- "11_genotype_scripts_zjap"
	reference_genome_location <- "/lustre/work/jmanthey/zosterops_genome/ref.fa"
	cluster <- "quanah"
	output_name <- "zjap_genotype"
	popmap <- "zjap_popmap.txt"
	individuals <- read.table(popmap, sep="\t")
	faidx <- read.table("ref.fa.fai", stringsAsFactors=F)
