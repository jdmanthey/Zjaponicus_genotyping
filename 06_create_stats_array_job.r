options(scipen=999)
project_directory <- "/lustre/scratch/jmanthey/02_zj/06_window"
directory_name <- "12_zj_stats"
cluster <- "nocona"
max_number_jobs <- 200
...
vcf_header <- paste0(project_directory, "/JABUOJ010027950.1.recode.vcf.gz")

# name of script to write
output_script_name <- "stat_array.sh"

# read in reference index
# filtered to only include genotyped chromosomes
ref_index <- read.table("jawe_big_scaffolds.fai", stringsAsFactors=F)
