...
calc_polymorphisms <- TRUE		# calculate private, shared, fixed polymorphisms in all pops with 
									# N >= calc_polymorphisms_minN
calc_titv <- FALSE				# calculate transition / transversion ratio of entire dataset
	...
	if(num_sites_needed <= nrow(xxx)) {
		...
		for(a in 1:nrow(genotypes)) {
			if(allele_info[a,2] == ".") { # if non-polymorphic
				genotypes[a,][genotypes[a,] == "0/0"] <- allele_info[a,1]
				...
				genotypes[a,][genotypes[a,] == "0/0"] <- allele_info[a,1]
				genotypes[a,][genotypes[a,] == "./."] <- "?"
				genotypes[a,][genotypes[a,] == "1/1"] <- allele_info[a,2]
				genotypes[a,][genotypes[a,] == "0/1"] <- 
					sample(het, length(genotypes[a,][genotypes[a,] == "0/1"]), replace=T)
			}
		}
...
if(make_FASTA == TRUE) {
	...
	output_fasta <- paste(strsplit(args[1], ".vcf")[[1]][1], ".fasta", sep="")
